  ...
  <div class="teacher-grading-batch">
    ...
    <el-card shadow="never" class="filter-card">
      <div class="filter-container">
        <el-select v-model="filterForm.courseId" placeholder="选择课程" class="filter-item">
          <el-option
        ...
        <el-select v-model="filterForm.assignmentId" placeholder="选择作业" class="filter-item">
          <el-option
        ...
        <el-select v-model="filterForm.questionId" placeholder="选择题目" class="filter-item">
          <el-option
    ...
    <el-card shadow="never" class="mt-4">
      ...
      <div class="batch-review-container">
        <div class="question-info">
          ...
          <p v-if="['single', 'multiple', 'truefalse'].includes(currentQuestion?.questionType || '')">
          ...
          >
            <div class="student-header">
              <div class="student-info">
                <el-checkbox v-model="student.selected" @change="handleStudentSelect"></el-checkbox>
                <span class="student-name">{{ student.studentName }}</span>
              </div>
              <div class="student-score">
                <span class="score-label">AI评分：</span>
                <span class="score-value">{{ student.aiScore }}/{{ currentQuestion?.score }}</span>
              </div>
            </div>
            
            <!-- 学生答案 -->
            <div class="answer-content">
              <div v-if="['single', 'multiple', 'truefalse'].includes(currentQuestion?.questionType || '')" class="objective-answer">
                <span>学生答案：{{ student.answer }}</span>
              </div>
              <div v-else class="subjective-answer" v-html="student.answer"></div>
            </div>
            
            <!-- AI反馈 -->
            <div v-if="currentQuestion?.questionType === 'subjective'" class="ai-feedback">
            ...
            <div class="teacher-actions">
              <el-button size="small" type="primary" @click="adoptScore(student.studentId)">采用</el-button>
              <el-button size="small" @click="modifyScore(student.studentId)">修改</el-button>
              <el-button size="small" @click="addComment(student.studentId)">添加评语</el-button>
            </div>
          </div>
        </div>
      </div>
...
const currentQuestion = computed(() => {
  return questions.value.find(q => q.questionId === parseInt(filterForm.value.questionId)) || null;
});
...
const batchAdopt = () => {
  const selectedStudents = studentsAnswers.value.filter(student => student.selected);
...
const batchModify = () => {
  ...
  ElMessageBox.prompt('请输入统一评分', '批量修改评分', {
    inputValidator: (value) => {
      ...
      if (isNaN(score) || score < 0 || score > (currentQuestion.value?.score || 100)) {
        return '请输入有效的评分';
      }
      return true;
    }
