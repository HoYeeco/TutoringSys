...
export const exportToPDF = (questions: ErrorQuestion[], filename: string = '错题本') => {
  const doc = new jsPDF();
  ...
  questions.forEach((q, index) => {
    ...
    doc.text(`第 ${index + 1} 题 (${q.courseName} - ${q.assignmentTitle})`, 14, yPos);
    yPos += 8;
    
    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    
    const questionLines = doc.splitTextToSize(`题目: ${q.questionContent}`, 180);
    ...
    const analysisLines = doc.splitTextToSize(`解析: ${q.analysis || '暂无解析'}`, 180);
...
export const exportToExcel = (questions: ErrorQuestion[], filename: string = '错题本') => {
  const worksheetData = [
    ['序号', '课程', '作业', '题目内容', '你的答案', '正确答案', '解析', '错误时间']
  ];
